...
# renames all the instances of the current MocapClipper with the prompted new name
# searches through files and replaces in there as well

# parent directory of the mocap-clipper
$TOOLS_DIR = (Get-Item -Path "..\").FullName

# define current tool name variables
$CURRENT_base_case = "mocap-clipper"
$CURRENT_snake_case = "mocap_clipper"
$CURRENT_PascalCase = "MocapClipper"
...
# Find instances of mocap_clipper in files and replace them with the new tool name
...
Foreach-Object {
    ...
    ((Get-Content -path $_.FullName -Raw) -replace "mocap-clipper", $NEW_base_case) | Set-Content -Path $_.FullName
